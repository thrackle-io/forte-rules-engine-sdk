...
```
npx typedoc --entryPointStrategy Expand src --tsconfig tsconfig.json
```
...
- All files in `src/example/` may also be licensed under `GPL-2.0-or-later` (as indicated in their SPDX headers), see [`LICENSE`](./LICENSE)
