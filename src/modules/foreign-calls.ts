...
export const createForeignCall = async (
    ...
    const parseddForeignCall = parseForeignCallDefinition(json)
    if (isRight(parseddForeignCall)) {
        const foreignCall = unwrapEither(parseddForeignCall);
        var fc = {
            set: true,
        ...
        throw new Error(unwrapEither(parseddForeignCall).message);
    }
}
/**
